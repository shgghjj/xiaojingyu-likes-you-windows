...
fun SettingsPanel(appState: AppState, configStore: ConfigStore, onDismiss: () -> Unit) {
    ...
    AlertDialog(
                ...
                Column(modifier = Modifier.weight(1f, fill = true).verticalScroll(rememberScrollState())) {
                    when (tab) {
                        0 -> {
                            ...
                            HorizontalDivider(color = Color(0xFF1A1A20), modifier = Modifier.padding(vertical = 6.dp))
                            ...
                            Text("授权工作目录（她可在此跑命令/改代码）", color = Color(0xFF9A9AA4), fontSize = 12.sp)
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                OutlinedTextField(authDirInput, { authDirInput = it }, Modifier.weight(1f), singleLine = true, placeholder = { Text("如 C:\\Projects\\myrepo") })
                                Spacer(Modifier.width(6.dp))
                                Button(onClick = {
                                    if (authDirInput.isNotBlank()) {
                                        authDirs = authDirs + authDirInput.trim()
                                        authDirInput = ""
                                    }
                        ...
                        1 -> {
                            appState.jailbreakPresets().forEach { preset ->
                                Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth().clickableCompat { jailbreakId = preset.id }) {
                                    ...
                                    Column {
                                        ...
                                        Text(preset.description, color = Color(0xFF6E6E7A), fontSize = 11.sp)
                                    }
                                }
                            }
                        ...
                        2 -> {
                            ...
                            if (facts.isEmpty()) Text("（空）", color = Color(0xFF6E6E7A), fontSize = 12.sp)
                            Spacer(Modifier.height(12.dp))
                            ...
                            if (memories.isEmpty()) Text("（空）", color = Color(0xFF6E6E7A), fontSize = 12.sp)
                        }
                    }
                }
            }
        ...
        confirmButton = {
            TextButton(onClick = {
                // 保存破甲词库
                if (jailbreakId == "custom" && customJailbreak.isNotBlank()) {
                    appState.saveCustomJailbreak(customJailbreak)
                } else {
                    appState.setJailbreak(jailbreakId)
                }
        ...
        dismissButton = {
            TextButton(onClick = onDismiss) { Text("取消", color = Color(0xFF6E6E7A)) }
        },
...
private fun SwitchRow(label: String, desc: String, checked: Boolean, onChange: (Boolean) -> Unit) {
    Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.weight(1f)) {
            ...
            Text(desc, color = Color(0xFF6E6E7A), fontSize = 11.sp)
        }
